...
function deleteUser($conn) {
    try {
        ...
        // Delete related records from cart_items
        $cart_stmt = $conn->prepare("DELETE FROM cart_items WHERE user_id = ?");
        $cart_stmt->bind_param("i", $user_id);
        $cart_stmt->execute();
        $cart_stmt->close();

        // Delete related records from reviews
        $reviews_stmt = $conn->prepare("DELETE FROM reviews WHERE user_id = ?");
...
function getUsers($conn) {
    $stmt = $conn->prepare("SELECT user_id, name, email, role, address, created_at FROM users ORDER BY created_at DESC");
    $stmt->execute();
    $result = $stmt->get_result();
